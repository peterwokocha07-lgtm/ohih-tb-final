...
# Optional: used by Password Reset + Facility Auto Registration
...
# =========================
# PAGE (1): PASSWORD RESET (Organizer) via RPC
# =========================
def page_password_reset():
    ...
    if st.button("Reset Password", type="primary"):
        ...
        try:
            ...
            try:
                data = r.json()
                df_show(pd.DataFrame(data if isinstance(data, list) else [data]), hide_index=True)
            except Exception:
                pass
        except Exception as e:
...
# =========================
# PAGE (3): AI WEIGHTS TUNING PANEL
# =========================
def page_ai_weights_tuning():
    ...
    with st.expander("Preview current weights JSON"):
        df_show(
            pd.DataFrame([st.session_state.get("ai_weights", {})])
            .T.reset_index()
            .rename(columns={"index": "key", 0: "weight"}),
            hide_index=True,
        )
...
def page_facility_auto_registration():
    ...
    st.caption(
        "This is **Step 4** (facility auto-registration workflow). "
        "You will paste the SQL RPC, then this page will create a facility + its first Facility Admin in one click."
    )

    st.markdown(
        """
**How it works (recommended):**
- Organizer enters facility details + the first Facility Admin account details.
- App calls RPC **auto_register_facility**.
- RPC should:
  1) Validate organizer_key
  2) Generate a unique facility_reg automatically
  3) Insert into facilities
  4) Create Auth user for the admin (requires service-role inside DB/Edge function approach)
  5) Insert staff_profiles row for that admin (role = facility_admin, facility_id = new facility)
  6) Return the created facility_id + facility_reg + admin credentials summary
"""
    )
    ...
    org_key = st.text_input("Organizer key", value=ORGANIZER_RESET_KEY, type="password")

    st.markdown("### 🏥 Facility details")
    c1, c2 = st.columns(2)
    with c1:
        facility_name = st.text_input("Facility name *", placeholder="e.g. PHC Central TB Clinic")
        state = st.text_input("State *", placeholder="e.g. Rivers")
        lga = st.text_input("LGA *", placeholder="e.g. Port Harcourt")
    with c2:
        latitude = st.text_input("Latitude (optional)", placeholder="e.g. 4.8156")
        longitude = st.text_input("Longitude (optional)", placeholder="e.g. 7.0498")
        st.caption("If you don't know coords now, leave blank and update later.")

    st.markdown("### 👤 First Facility Admin (account to be created)")
    c3, c4 = st.columns(2)
    with c3:
        admin_full_name = st.text_input("Admin full name *", placeholder="e.g. TB Focal Person")
        admin_email = st.text_input("Admin email *", placeholder="e.g. tbadmin@facility.ng")
        admin_staff_id = st.text_input("Admin staff ID *", placeholder="e.g. TB-001")
    with c4:
        admin_profession = st.text_input("Admin profession (optional)", placeholder="e.g. Nurse / Doctor / M&E")
        gen_pw = st.button("Generate strong temp password")
        if gen_pw:
            st.session_state["auto_reg_pw"] = random_password(12)
        admin_temp_password = st.text_input(
            "Admin temp password *",
            value=st.session_state.get("auto_reg_pw", ""),
            type="password",
    ...
    if st.button("AUTO-REGISTER FACILITY NOW", type="primary"):
        ...
        if not org_key.strip():
            ...
            st.stop()
        if not facility_name.strip() or not state.strip() or not lga.strip():
            st.error("Facility name, state, and LGA are required.")
            st.stop()
        if not admin_full_name.strip() or not admin_email.strip() or not admin_staff_id.strip():
            st.error("Admin full name, email, and staff ID are required.")
            st.stop()
        if not admin_temp_password.strip():
        ...
        if latitude.strip():
            try:
                lat_v = float(latitude.strip())
            except Exception:
                st.error("Latitude must be a number.")
                st.stop()
        if longitude.strip():
            try:
                lon_v = float(longitude.strip())
            except Exception:
                st.error("Longitude must be a number.")
                st.stop()

        payload = {
            "organizer_key": org_key.strip(),
            ...
            "admin_staff_id": admin_staff_id.strip(),
            "admin_profession": admin_profession.strip(),
            "admin_temp_password": admin_temp_password,
        }

        tok = st.session_state["access_token"]
        try:
            r = rpc_call("auto_register_facility", tok, payload)
            if r.status_code not in (200, 201):
                st.error(f"RPC failed: {r.status_code} {r.text}")
                st.stop()

            st.success("Facility auto-registration successful ✅")
            data = r.json()
            df_show(pd.DataFrame(data if isinstance(data, list) else [data]), hide_index=True)

            st.info(
                "✅ NEXT: Give the admin these credentials to log in.\n\n"
                "- Admin should change password after first login (recommended).\n"
                "- If you also want an automated password-change flow, we will add it in next upgrade."
            )
        except Exception as e:
            st.error("Auto-registration error.")
            st.exception(e)
